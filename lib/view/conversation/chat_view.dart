...
class _ChatViewState extends State<ChatView> {
  ...
  Future<void> fetchConversation() async {
    ChatClient.getInstance.chatManager
      ...
      conversationId: _receiver.metadata!['phone'],
      pageSize: 20,
    )
        .then((cursor) {
    ...
    ChatClient.getInstance.chatManager.addEventHandler(
      ...
      ChatEventHandler(
        ...
        onMessagesDelivered: (messages) {
          print('onMessagesDelivered: ${messages.length}');
          _onMessagesDelivered(messages);
        },
        onMessagesRead: (messages) {
          print('onMessagesRead: ${messages.length}');
          _onMessagesDelivered(messages);
        },
      ),
    );
  ...
  void _onMessagesReceived(List<ChatMessage> messages) async {
    for (var msg in messages) {
  ...
  chat_types.Status _convertRemoteStatusToLocalStatus(ChatMessage message) {
    if (message.hasRead) {
      return chat_types.Status.seen;
    }
    return chat_types.Status.sent;
  }
    ...
    switch (message.body.type) {
      case MessageType.IMAGE:
        ...
        insertMessage = chat_types.ImageMessage(
          status: _convertRemoteStatusToLocalStatus(message),
          author:
              _sender.metadata!['phone'] == message.from ? _sender : _receiver,
          id: message.msgId,
  ...
  void _sendMessage(String? content) async {
    ...
    var msg = ChatMessage.createTxtSendMessage(
      targetId: _receiver.metadata!['phone'],
      content: content,
    );
                    ...
                    suffixIcon: GestureDetector(
                      onTap: () {
                        ...
                        showDialog(
                          ...
                          builder: (context) {
                            return AlertDialog(
                              content: const Text('Please select the source'),
                              actions: [
                                TextButton(
                                    onPressed: () {
